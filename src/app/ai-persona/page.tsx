...
function isValidBangladeshPhone(value: string) {
  return BANGLADESH_PHONE_REGEX.test(normalizeBangladeshPhone(value));
}

export default function AiPersonaHome() {
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch('/api/auth/otp/send', {
        ...
        body: JSON.stringify({ name, phone: normalizedPhone, dob }),
      });
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch('/api/auth/otp/verify', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name, phone: normalizedPhone, dob, otp }),
      });
      const data = await res.json();
              ...
              <form onSubmit={handleSendOtp}>
                ...
                <div className={styles.formGroup}>
                  ...
                  <input
                    ...
                    placeholder="e.g. 017XXXXXXXX or +8801XXXXXXXXX"
                    inputMode="tel"
                    required
                  />
                </div>
